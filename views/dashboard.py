from asciimatics.widgets import Frame, Layout, Divider, VerticalDivider, Button
...
class Dashboard(Frame):
    def __init__(self, screen):
        ...
        main.add_widget(Button('EtherToken', self.detail(S['ETHER_TOKEN'])), 0)
        main.add_widget(Divider(line_char=''))
        main.add_widget(Button('MarketToken', self.detail(S['MARKET_TOKEN'])), 0)
    ...
    def detail(self, which):
        """
        return a curried function to move from the dashboard view to a detail view
        """
        def c():
